...
const createGenesisBlock = (gameLobbyID): Block => {
...
const getBlockchain = (gameLobbyID): Block[] => blockchains[gameLobbyID];
// 마지막 블록의 정보를 가지고 오는 과정이다. 현 체인의 길이에서 - 1 을 한 index 를 가지고 있는 블록의 정보를 가지고 온다.
const getLatestBlock = (gameLobbyID): Block => blockchains[gameLobbyID][blockchains[gameLobbyID].length - 1];
...
// 블록을 생성하는 과정이다. 
const generateNextBlock = (blockData: string, gameLobbyID) => {
...
// 새로운 블록을 더하는 과정이다.
const addBlock = (newBlock: Block, gameLobbyID) => {
...
const addBlockToChain = (newBlock: Block, gameLobbyID) => {
    if (isValidNewBlock(newBlock, getLatestBlock(gameLobbyID))) {
        blockchains[gameLobbyID].push(newBlock);
        return true;
    }
...
const replaceChain = (newBlocks: Block[], gameLobbyID) => {
